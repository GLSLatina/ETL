...
#		 if ( (test "$PESOCLEAN" -ge 3) || (test "$PEVOCLEAN" -ge 3) );then
		 if (test -n "$PESOMORETHANLIGHT" ); then
			...
			echo  SPED:$nsped MITT:"$mitt" DEST:"$dindi" $vdprov $peso $pevo
		 fi
	...
	LIGHT=0
		PESOLESSTHANLIGHT=`echo $PESOCLEAN $PEVOCLEAN | sed -e 's/^,/0,/g' -e 's/,.*$//' | awk 'gsub(",",".",$1)  gsub(",",".",$2) && ($1<3.6 && $2<3.6) && ($1!=0 && $2!=0) { print $peso,$pevo }' `
		 ...
		 if (test -n "$PESOLESSTHANLIGHT" ); then
			...
			echo  SPED:$nsped MITT:"$mitt" DEST:"$dindi" $vdprov $peso $pevo $PESOCLEAN $PEVOCLEAN
		fi
fi
...
# NON CERTIFICATA MODIFICABILE AUTOMATICAMENTE - NON LIGHT
		...
		#echo PESOCLEAN: $PESOCLEAN PEVOCLEAN: $PEVOCLEAN PESOAZERO: $PESOAZERO
			...
			echo  SPED:$nsped CERT: "$TESTCERTIFICA" MITT:"$mitt" DEST:"$dindi" $vdprov $PESOCLEAN $PEVOCLEAN
		fi
...
# Manca Email Mittente
		...
		echo  SPED:$nsped  MITT:"$mitt" DEST:"$dindi" $vdprov $PESOCLEAN $PEVOCLEAN MAIL MIT: "$TESTEMAILMIT"
fi

# Manca Email Destinatario
		 if  (test -z "$TESTEMAILDEST" ) ; then
			echo -n NON-EMAIL-DEST:
		echo  SPED:$nsped  MITT:"$mitt" DEST:"$dindi" $vdprov $PESOCLEAN $PEVOCLEAN MAIL DST: "$TESTEMAILDEST"
fi

# Manca Tel Mittente
		 if  (test -z "$TESTTELMIT" ) ; then
			echo -n NON-TEL-MITT:
		echo  SPED:$nsped  MITT:"$mitt" DEST:"$dindi" $vdprov $PESOCLEAN $PEVOCLEAN TEL MIT: "$TESTTELMIT"
fi

# Manca Tel Destinatario
		 if  (test -z "$TESTTELDEST" ) ; then
			echo -n NON-TEL-DEST:
		echo  SPED:$nsped  MITT:"$mitt" DEST:"$dindi" $vdprov $PESOCLEAN $PEVOCLEAN TEL DST: "$TESTTELDEST"
fi
